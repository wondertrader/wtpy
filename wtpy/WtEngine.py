...
class WtEngine:
    ...
    def add_sel_strategy(self, strategy: BaseSelStrategy, date: int, time: int, period: str, trdtpl: str = "CHINA",
                         session: str = "TRADING", slippage: int = 0):
        """
        添加SEL策略
        @ strategy  SEL策略对象
        ...
        @slippage   滑点大小
        """
        id = self.__wrapper__.create_sel_context(name=strategy.name(), date=date, time=time, period=period,
                                                 trdtpl=trdtpl, session=session, slippage=slippage)
