...
class WtBtEngine:
    ...
    def init_with_config(self, folder: str,
        ...
        if self.__config__["replayer"]["basefiles"]["commodity"] is not None:
            if isinstance(self.__config__["replayer"]["basefiles"]["commodity"], str):
                self.productMgr.load(self.__config__["replayer"]["basefiles"]["commodity"])
            elif isinstance(self.__config__["replayer"]["basefiles"]["commodity"], list):
        ...
        if isinstance(self.__config__["replayer"]["basefiles"]["contract"], str):
            self.contractMgr.load(self.__config__["replayer"]["basefiles"]["contract"])
        elif isinstance(self.__config__["replayer"]["basefiles"]["contract"], list):
    ...
    def init(self, folder: str,
        ...
        if cfgfile.lower().endswith(".json"):
            self.init_with_config(folder, json.loads(content), commfile, contractfile, sessionfile, holidayfile,
                                  hotfile, secondfile)
            self.__is_cfg_yaml__ = False
        else:
            self.init_with_config(folder, yaml.full_load(content), commfile, contractfile, sessionfile, holidayfile,
                                  hotfile, secondfile)
    ...
    def set_cta_strategy(self, strategy: BaseCtaStrategy, slippage: int = 0, hook: bool = False,
                         persistData: bool = True, incremental: bool = False, isRatioSlp: bool = False):
        """
        添加CTA策略
        @strategy   策略对象
        @slippage   滑点大小
    ...
    def set_sel_strategy(self, strategy: BaseSelStrategy, date: int = 0, time: int = 0, period: str = "d",
                         trdtpl: str = "CHINA", session: str = "TRADING", slippage: int = 0, isRatioSlp: bool = False):
        """
        添加SEL策略
        @strategy   策略对象
