...
class WtWrapper:
    ...
    def on_hftstra_position(self, id: int, stdCode: str, isLong: bool, prevol: float, preavail: float, newvol: float,
                            newavail: float):
        stdCode = bytes.decode(stdCode)
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def on_hftstra_get_transaction(self, d: int, stdCode: str, newTrans: POINTER(WTSTransStruct), count: int,
                                   isLast: bool):
        engine = self._engine
        ctx = engine.get_context(id)
    ...
    def initialize_cta(self, logCfg: str = "logcfg.yaml", isFile: bool = True, genDir: str = 'generated'):
        ...
        try:
            ...
            self.api.register_cta_callbacks(self.cb_stra_init, self.cb_stra_tick, self.cb_stra_calc, self.cb_stra_bar,
                                            self.cb_session_event, self.cb_stra_cond_trigger)
    ...
    def sel_get_bars(self, id: int, stdCode: str, period: str, count: int, isMain: bool):
        ...
        @isMain     是否主K线
        """
        return self.api.sel_get_bars(id, bytes(stdCode, encoding="utf8"), bytes(period, encoding="utf8"), count, isMain,
                                     CB_STRATEGY_GET_BAR(self.on_stra_get_bar))
