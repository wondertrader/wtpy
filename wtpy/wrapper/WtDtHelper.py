...
from wtpy.WtDataDefs import WtTickCache, WtNpOrdDetails, WtNpOrdQueues, WtNpTransactions, WtNpKline, WtNpTicks, \
    WtBarCache
...
class WtDataHelper:
    ...
    def __init__(self):
        ...
        self.api.resample_bars.argtypes = [c_char_p, CB_DTHELPER_BAR, CB_DTHELPER_COUNT, c_uint64, c_uint64, c_char_p,
                                           c_uint32, c_char_p, CB_DTHELPER_LOG]
    ...
    def read_dsb_order_details(self, dataFile: str) -> WtNpOrdDetails:
        ...
        if 0 == self.api.read_dsb_order_details(bytes(dataFile, encoding="utf8"),
                                                CB_DTHELPER_ORDDTL(data_cache.on_read_data),
                                                CB_DTHELPER_COUNT(data_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
    ...
    def read_dsb_order_queues(self, dataFile: str) -> WtNpOrdQueues:
        ...
        data_cache = DataCache()
        if 0 == self.api.read_dsb_order_queues(bytes(dataFile, encoding="utf8"),
                                               CB_DTHELPER_ORDQUE(data_cache.on_read_data),
                                               CB_DTHELPER_COUNT(data_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return data_cache.records
    ...
    def read_dsb_transactions(self, dataFile: str) -> WtNpTransactions:
        ...
        data_cache = DataCache()
        if 0 == self.api.read_dsb_transactions(bytes(dataFile, encoding="utf8"),
                                               CB_DTHELPER_TRANS(data_cache.on_read_data),
                                               CB_DTHELPER_COUNT(data_cache.on_data_count), self.cb_dthelper_log):
            return None
        else:
            return data_cache.records

    def resample_bars(self, barFile: str, period: str, times: int, fromTime: int, endTime: int, sessInfo: SessionInfo,
                      alignSection: bool = False) -> WtNpKline:
        """
        重采样K线
        ...
        if 0 == self.api.resample_bars(bytes(barFile, encoding="utf8"), CB_DTHELPER_BAR(bar_cache.on_read_bar),
                                       ...
                                       bytes(sessInfo.toString(), 'utf8'), self.cb_dthelper_log, alignSection):
            return None
        else:
            return bar_cache.records
