...
class WtDtServoApi:
    ...
    def get_bars(self, stdCode: str, period: str, fromTime: int = None, dataCount: int = None,
        ...
        if fromTime is not None:
            ret = self.api.get_bars_by_range(bytes(stdCode, encoding="utf8"), bytes(period, 'utf8'), fromTime, endTime,
                                             CB_GET_BAR(bar_cache.on_read_bar), CB_DATA_COUNT(bar_cache.on_data_count))
        else:
    ...
    def get_ticks(self, stdCode: str, fromTime: int = None, dataCount: int = None, endTime: int = 0) -> WtNpTicks:
        ...
        if fromTime is not None:
            ret = self.api.get_ticks_by_range(bytes(stdCode, encoding="utf8"), fromTime, endTime,
                                              ...
                                              CB_DATA_COUNT(tick_cache.on_data_count))
        else:
    ...
    def get_ticks_by_date(self, stdCode: str, iDate: int) -> WtNpTicks:
        ...
        """
        tick_cache = WtTickCache()
        ret = self.api.get_ticks_by_date(bytes(stdCode, encoding="utf8"), iDate, CB_GET_TICK(tick_cache.on_read_tick),
                                         CB_DATA_COUNT(tick_cache.on_data_count))
    ...
    def get_sbars_by_date(self, stdCode: str, iSec: int, iDate: int) -> WtNpKline:
        ...
        """
        bar_cache = WtBarCache()
        ret = self.api.get_sbars_by_date(bytes(stdCode, encoding="utf8"), iSec, iDate,
                                         CB_GET_BAR(bar_cache.on_read_bar), CB_DATA_COUNT(bar_cache.on_data_count))
    ...
    def get_bars_by_date(self, stdCode: str, period: str, iDate: int) -> WtNpKline:
        ...
        bar_cache = WtBarCache()
        ret = self.api.get_bars_by_date(bytes(stdCode, encoding="utf8"), bytes(period, encoding="utf8"), iDate,
                                        CB_GET_BAR(bar_cache.on_read_bar), CB_DATA_COUNT(bar_cache.on_data_count))
