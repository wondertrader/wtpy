...
class WtCacheMonExchg(WtCacheMon):
    ...
    def getDceData(curDT: datetime.datetime) -> dict:
        ...
        try:
            data = {'dayQuotes.variety': 'all', 'dayQuotes.trade_type': 0, 'year': curDT.year, 'month': curDT.month - 1,
                    'day': curDT.day}
            ...
            if items is not None:
                ...
                for key in hots.keys():
                    ...
                    if key not in lastHots:
                        item = {"date": int(curDT.strftime('%Y%m%d')), "from": "", "to": hots[key], "oldclose": 0.0,
                                "newclose": items[hots[key]].close}
                        ...
                        if oldItem is None or newItem.month > oldItem.month:
                            item = {"date": int(nextDT.strftime('%Y%m%d')), "from": oldcode, "to": newcode}
                            if oldcode in items:
                ...
                for key in seconds.keys():
                    ...
                    if key not in lastSecs:
                        item = {"date": int(curDT.strftime('%Y%m%d')), "from": "", "to": seconds[key], "oldclose": 0.0,
                                "newclose": items[seconds[key]].close}
