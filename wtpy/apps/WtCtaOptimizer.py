...
def start_task_group(env_params, gpName: str, params: list, counter, capital=5000000, rf=0, period=240,
    ...
    if env_params["iscfgfile"]:
        ...
        engine.init_with_config(env_params["deps_dir"], env_params["cfgfile"],
                                ...
                                env_params["deps_files"]["sessionfile"], env_params["deps_files"]["holidayfile"],
                                env_params["deps_files"]["hotfile"], env_params["deps_files"]["secondfile"])
    engine.configBTStorage(mode=env_params["storage_type"], path=env_params["storage_path"],
                           storage=env_params["storage"])
    # 遍历参数组
    total = len(params)
    cnt = 0
    for param in params:
...
class WtCtaOptimizer:
    ...
    def go(self, order_by_field: str = "", out_marker_file: str = "strategies.json",
        ...
        for i in range(max_cnt):
            ...
            p = multiprocessing.Process(
                ...
                args=(self.env_params, work_name, params, self.counter, capital, rf, period, self.strategy_type,
                      self.cpp_stra_module, self.cpp_stra_type),
                name=work_name)
            p.start()
        ...
        while alive_cnt > 0:
            ...
            if self.notifier is not None:
                ...
                self.notifier.on_state(total_size, self.counter.value, self.counter.value * 100 / total_size,
                                       int(elapse.total_seconds() * 1000))
            time.sleep(0.5)
    ...
    def analyzer(self, out_marker_file: str = "strategies.json", init_capital=500000, rf=0.02, annual_trading_days=240):
        for straname in json.load(open(out_marker_file, mode='r')).keys():
            try:
                analyst = WtBtAnalyst()
                analyst.add_strategy(straname, folder=f"./outputs_bt/{straname}/", init_capital=init_capital, rf=rf,
