...
class WtHftOptimizer:
    ...
    def __ayalyze_result__(self, strName: str, time_range: tuple, params: dict):
        ...
        df_closes["opentime"] = df_closes["opentime"].astype("str").apply(
            lambda dt: datetime.datetime.strptime(dt, "%Y%m%d%H%M%S%f"))
        df_closes["closetime"] = df_closes["closetime"].astype("str").apply(
            lambda dt: datetime.datetime.strptime(dt, "%Y%m%d%H%M%S%f"))
        ...
        sr = sr.round(3)
        df_funds["dd"] = (
                    df_funds['dynbalance'].rolling(len(df_funds) + 1, min_periods=1).max() - df_funds['dynbalance'])
